...
<template>
  <div>
    <h1>Custom Party Filter</h1>
    <div v-for="(filter, index) in filterData" :key="index">
      <component v-if="filter.type === 'select'" :is="'v-select'" v-model="filter.value" :multiple="filter.attrs.multiple" :label="filter.attrs.label" :items="filter.attrs.items" :rules="filter.attrs.rules"></component>
