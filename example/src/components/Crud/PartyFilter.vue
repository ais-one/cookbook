...
<template>
  <div>
    <div v-for="(filter, index) in filterData" :key="index">
      <component v-if="filter.type === 'v-select'" :is="'v-select'" v-model="filter.value" :multiple="filter.multiple" :label="filter.label" :items="filter.items" :rules="filter.rules"></component>
