...
exports.setRefreshToken = async (id, refresh_token) => await mongo.db.collection(JWT_REFRESH_STORE_NAME).updateOne({ _id: new ObjectID(id) }, { $set: { refresh_token, setAt: new Date() } }, { upsert: true, w: 1, j: true, wtimeout: 2000 })
...
exports.revokeRefreshToken = async(id) => await mongo.db.collection(JWT_REFRESH_STORE_NAME).deleteOne({ _id: new ObjectID(id) })

exports.findUser = async (where) => await mongo.db.collection(AUTH_USER_STORE_NAME).findOne((where.id ? { _id: new ObjectID(where.id) } : where))
exports.updateUser = async (where, payload) => await mongo.db.collection(AUTH_USER_STORE_NAME).updateOne(where.id ? { _id: new ObjectID(where.id) } : where, { $set: payload })
