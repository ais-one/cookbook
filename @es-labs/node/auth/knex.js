...
exports.setRefreshToken = async (id, refresh_token) => await knex(JWT_REFRESH_STORE_NAME).insert({ id, refresh_token }).onConflict('id').merge()
exports.getRefreshToken = async (id) => ( await knex(JWT_REFRESH_STORE_NAME).where({ id: id }).first() ).refresh_token
exports.revokeRefreshToken = async(id) => await knex(JWT_REFRESH_STORE_NAME).where({ id: id }).delete()

exports.findUser = async (where) => await knex(AUTH_USER_STORE_NAME).where(where).first()
exports.updateUser = async (where, payload) => await knex(AUTH_USER_STORE_NAME).where(where).first().update(payload)
